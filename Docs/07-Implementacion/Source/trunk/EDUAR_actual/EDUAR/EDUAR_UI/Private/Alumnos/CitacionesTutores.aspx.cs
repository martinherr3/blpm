...
namespace EDUAR_UI
{
	public partial class CitacionesTutores : EDUARBasePage
	{
		#region --[Atributos]--
		...
		public List<Citacion> listaCitaciones
		{
			get
			{
				if (Session["listaCitaciones"] == null)
					listaCitaciones = new List<Citacion>();

				return (List<Citacion>)Session["listaCitaciones"];
			}
			set { Session["listaCitaciones"] = value; }
		}
		...
		private void CargarLista(Citacion entidad)
		{
			...
			if (entidad == null) entidad = new Citacion();
		...
		private void CargarValoresEnPantalla(int idCitacion)
		{
			...
			if (entidad != null)
			{
				lblDescripcion.Text = entidad.detalles;
				CargarCombosEdicion();
