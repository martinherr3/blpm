 ...
 * @author 1
 */
	var strFoundNothing = "No se ha encontrado nada";
	var strSearchStringIsEmpty = "Introduzca palabras de búsqueda";
	var strSearchInProgress = "Buscando...";
...
function isEmpty(sToCheck) {
   ...
   return false;
	...
	function performSearch(queryArray)
	{
...
function onHighlight(){
	...
	if (document.getElementById('search').value != '')
	{
		var s = trim(document.getElementById('search').value).replace(/\r/g,'').replace(/\n/g,'').replace(/\t/g,' ').replace(/\u00A0/g,'');
		s = s.replace(/[,;]/g, ' ');
		var strArray = trim(s).split(querySplitter);
