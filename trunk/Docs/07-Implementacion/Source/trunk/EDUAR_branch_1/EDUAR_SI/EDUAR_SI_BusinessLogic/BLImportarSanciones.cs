...
{
	public class BLImportarSanciones : BLProcesoBase
	{
		#region --[Atributos]--
		...
		public BLImportarSanciones(String connectionString)
		...
		public void ImportarSanciones()
		{
			try
			{
				...
				ProcesosEjecutadosCreate(enumProcesosAutomaticos.ImportarSanciones.GetHashCode(), true);
			}
			catch (Exception ex)
			{
				...
				OnErrorProcess(enumProcesosAutomaticos.ImportarSanciones.GetHashCode(), ex);
				throw ex;
			}
		}
		#endregion
		...
		private void ImportarDatos()
		{
			...
			try
			{
				...
				SqlConnection conexion = new SqlConnection() { ConnectionString = ConnectionString };
				{
					...
					objDAImportarDatos.GrabarMotivoSancion(objDAObtenerDatos.obtenerMotivoSancionBDTransaccional(objConfiguracion), transaccion);

					objDAImportarDatos.GrabarTipoSancion(objDAObtenerDatos.obtenerTipoSancionBDTransaccional(objConfiguracion), transaccion);

					objDAImportarDatos.GrabarSancion(objDAObtenerDatos.obtenerSancionBDTransaccional(objConfiguracion), transaccion);
				}
