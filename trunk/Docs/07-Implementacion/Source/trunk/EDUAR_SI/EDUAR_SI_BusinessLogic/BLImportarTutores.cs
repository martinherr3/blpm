...
{
	public class BLImportarTutores : BLProcesoBase
	{
		#region --[Atributos]--
		...
		public BLImportarTutores(String connectionString)
		...
		public void ImportarTutores()
		{
			try
			{
				...
				ProcesosEjecutadosCreate(enumProcesosAutomaticos.ImportarTutores.GetHashCode(), true);
			}
			catch (Exception ex)
			{
				...
				OnErrorProcess(enumProcesosAutomaticos.ImportarTutores.GetHashCode(), ex);
				throw ex;
			}
		}
		#endregion
		...
		private void ImportarDatos()
		{
			...
			try
			{
				...
				SqlConnection conexion = new SqlConnection() { ConnectionString = ConnectionString };
				{
					...
					objDAImportarDatos.GrabarTipoTutor(objDAObtenerDatos.obtenerTipoTutorBDTransaccional(objConfiguracion), transaccion);
					
					//User Story 141
					GrabarTutor(transaccion);

					objDAImportarDatos.GrabarTutoresAlumno(objDAObtenerDatos.obtenerTutoresAlumnoBDTransaccional(objConfiguracion), transaccion);
				}
