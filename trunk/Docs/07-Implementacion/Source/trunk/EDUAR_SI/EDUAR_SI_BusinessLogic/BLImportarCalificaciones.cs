...
{
	public class BLImportarCalificaciones: BLProcesoBase
	{
		#region --[Atributos]--
		...
		public BLImportarCalificaciones(String connectionString)
		...
		/// </summary>
		public void ImportarCalificaciones()
		{
			try
			{
				...
				ProcesosEjecutadosCreate(enumProcesosAutomaticos.ImportarCalificaciones.GetHashCode(), true);
			}
			catch (Exception ex)
			{
				...
				OnErrorProcess(enumProcesosAutomaticos.ImportarCalificaciones.GetHashCode(), ex);
				throw ex;
			}
		}
		#endregion
		...
		private void ImportarDatos()
		{
			...
			try
			{
				...
				SqlConnection conexion = new SqlConnection() { ConnectionString = ConnectionString };
				{
					...
					objDAImportarDatos.GrabarCalificacion(objDAObtenerDatos.obtenerCalificacionBDTransaccional(objConfiguracion), transaccion);
				}
