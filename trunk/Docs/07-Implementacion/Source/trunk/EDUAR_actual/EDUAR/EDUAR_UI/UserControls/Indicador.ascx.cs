...
namespace EDUAR_UI.UserControls
{
    public partial class Indicador : UserControl
    {
        ...
        /// <summary>
        /// Gets or sets the curso indicador.
        /// </summary>
        /// <value>
        /// The curso indicador.
        /// </value>
        public string cursoIndicador
        {
            get
            {
                if (ViewState["cursoIndicador_" + this.UniqueID] != null)
                    return ViewState["cursoIndicador_" + this.UniqueID].ToString();
                return string.Empty;
            }
            set { ViewState["cursoIndicador_" + this.UniqueID] = value; }
        }
        ...
        {
            if (!string.IsNullOrEmpty(cursoIndicador))
            {
                lblCurso.Text = cursoIndicador;
        ...
        private void SetIndicadores()
        {
            if (!String.IsNullOrEmpty(nombreSP))
            {
                ...
                decimal valor = objBLIndicadores.GetValorIndicador("Indicadores_" + nombreSP, idCursoIndicador, DateTime.Today.AddDays(HastaPrincipal * -1), DateTime.Today);
                btnIndicador.Text = valor.ToString();
                btnIndicador.CommandArgument = this.ID;

                valor = objBLIndicadores.GetValorIndicador("Indicadores_" + nombreSP, idCursoIndicador, DateTime.Today.AddDays(HastaIntermedio * -1), DateTime.Today);
                btnIntermedio.Text = valor.ToString();
                btnIntermedio.CommandArgument = this.ID;

                valor = objBLIndicadores.GetValorIndicador("Indicadores_" + nombreSP, idCursoIndicador, DateTime.Today.AddDays(HastaSecundario * -1), DateTime.Today);
