...
namespace EDUAR_UI.UserControls
{
    public partial class Calendario : UserControl
    {
        ...
        {
            ValidarFormatoDesdeEx();
            ComprobarRangoMaxMin();
            ComprobarRangosDesde(comprobarFechaHoy);
        }

        /// <summary>
        /// Comprueba que las fechas ingresadas se correspondan con el intervalo StartDate y EndDate si están definidos.
        /// </summary>
        /// <exception cref="CustomizedException">
        /// </exception>
        private void ComprobarRangoMaxMin()
        {
            DateTime fechaDesde;
            DateTime fechaHasta;
            ...
            {
                DateTime.TryParse(txtFecha.Text.Trim(), out fechaDesde);
                DateTime.TryParse(txtFecha.Text.Trim(), out fechaHasta);
                if (startDate.HasValue && fechaDesde.CompareTo(Convert.ToDateTime(startDate)) < 0)
                    throw new CustomizedException(string.Format("{0} debe ser mayor o igual a {1}.", this.lblFechaDesde.Text.Replace(":", ""), Convert.ToDateTime(startDate).ToShortDateString()), null, enuExceptionType.ValidationException);
                if (endDate.HasValue && fechaHasta.CompareTo(Convert.ToDateTime(endDate)) > 0)
                    throw new CustomizedException(string.Format("{0} debe ser menor o igual a {1}.", this.lblFechaDesde.Text.Replace(":", ""), Convert.ToDateTime(endDate).ToShortDateString()), null, enuExceptionType.ValidationException);
            }
            else
            {
                DateTime.TryParse(txtFechaDesde_DA.Text.Trim(), out fechaDesde);
                DateTime.TryParse(txtFechaHasta_DA.Text.Trim(), out fechaHasta);
                if (startDate.HasValue && fechaDesde.CompareTo(Convert.ToDateTime(startDate)) < 0)
                    throw new CustomizedException(string.Format("{0} debe ser mayor o igual a {1}.", this.lblFechaDesde_DA.Text.Replace(":", "") + " ", Convert.ToDateTime(startDate).ToShortDateString()), null, enuExceptionType.ValidationException);
                if (endDate.HasValue && fechaHasta.CompareTo(Convert.ToDateTime(endDate)) < 0)
                    throw new CustomizedException(string.Format("{0} debe ser menor o igual a {1}.", this.lblFechaHasta_DA.Text.Replace(":", "") + " ", Convert.ToDateTime(endDate).ToShortDateString()), null, enuExceptionType.ValidationException);
            }
        }
